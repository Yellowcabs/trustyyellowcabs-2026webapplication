...
export const LocationPage: React.FC = () => {
  ...
  const locationData = LOCATIONS_DATA.find((loc) => loc.slug === slug);
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="flex flex-col space-y-10 text-center lg:text-left order-2 lg:order-1">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg w-fit mx-auto lg:mx-0 shadow-sm">
                ...
                <span className="text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest">Trustyyellowcabs {locationData.name}</span>
              </div>
      ...
      <section className="py-24 bg-white dark:bg-slate-950">
        <div className="container mx-auto px-4 lg:px-12">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-8 tracking-tight">
              Best Taxi Service in {locationData.name}
            </h2>
            <div className="prose prose-slate dark:prose-invert max-w-none">
              ...
              <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed mb-6">
                Trusty Yellow Cabs is dedicated to providing the most reliable and affordable taxi service in {locationData.name}. Our commitment to customer satisfaction is reflected in our professional drivers, well-maintained fleet, and transparent pricing. Whether you are a local resident or a visitor, we ensure that your travel needs are met with the highest standards of safety and comfort.
              </p>
              <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed mb-6">
                Our services in {locationData.name} include local city rides, airport transfers, and outstation trips. We understand the unique requirements of our customers and offer flexible booking options. With our 24/7 availability, you can rest assured that a cab is always available when you need one. Our drivers are trained to handle various road conditions and provide a smooth journey, whether it's a short trip within the city or a long-distance drive to a nearby hill station.
              </p>
              <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
                Booking a cab with us is simple and convenient. You can use our online booking form or give us a call. We offer various vehicle types, including Mini, Sedan, and SUV, to accommodate different group sizes and preferences. Experience the best taxi service in {locationData.name} with Trusty Yellow Cabs. Your safety and comfort are our top priorities.
              </p>
            </div>
          </div>
        </div>
